...
local SaveManager = {} do
    ...
    function SaveManager:RefreshConfigList()
        ...
        if (not success) then
            if self.Library then
                self.Library:Notify('[save] failed to load config list: ' .. tostring(data))
            else
    ...
    function SaveManager:LoadAutoloadConfig()
        ...
        if isfile(autoLoadPath) then
            local successRead, name = pcall(readfile, autoLoadPath)
            if not successRead then
                self.Library:Notify('[save] failed to load autoload config: write file error')
                return
            end

            local success, err = self:Load(name)
            if not success then
                self.Library:Notify('[save] failed to load autoload config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] auto loaded config %q', name))
        end
    end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton('create config', function()
            ...
            if name:gsub(' ', '') == '' then
                self.Library:Notify('[save] invalid config name (empty)', 2)
                return
            end

            local success, err = self:Save(name)
            if not success then
                self.Library:Notify('[save] failed to create config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] created config %q', name))
        ...
        section:AddButton('load config', function()
            ...
            local success, err = self:Load(name)
            if not success then
                self.Library:Notify('[save] failed to load config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] loaded config %q', name))
        end)
        section:AddButton('overwrite config', function()
            ...
            local success, err = self:Save(name)
            if not success then
                self.Library:Notify('[save] failed to overwrite config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] overwrote config %q', name))
        end)
        ...
        section:AddButton('delete config', function()
            ...
            if not success then
                self.Library:Notify('[save] failed to delete config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] deleted config %q', name))
            self.Library.Options.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
            self.Library.Options.SaveManager_ConfigList:SetValue(nil)
        end)
        ...
        section:AddButton('set as autoload', function()
            ...
            if not success then
                self.Library:Notify('[save] failed to set autoload config: ' .. err)
                return
            end

            self.Library:Notify(string.format('[save] set %q to auto load', name))
        ...
        section:AddButton('reset autoload', function()
            ...
            self.Library:Notify('[save] set autoload to none')
