...
local ThemeManager = {} do
	...
	function ThemeManager:SaveCustomTheme(file)
		if file:gsub(' ', '') == '' then
			self.Library:Notify('[theme] invalid file name for theme (empty)', 3)
			return
		end
	...
	function ThemeManager:CreateThemeManager(groupbox)
		...
		groupbox:AddButton('set as default', function()
			...
			self.Library:Notify(string.format('[theme] set default theme to %q', self.Library.Options.ThemeManager_ThemeList.Value))
		end)
		...
		groupbox:AddButton('create theme', function() 
			...
			if name:gsub(" ", "") == "" then
                self.Library:Notify("[theme] invalid theme name (empty)", 2)
                return
            end

            self:SaveCustomTheme(name)

            self.Library:Notify(string.format("[theme] created theme %q", name))
		...
		groupbox:AddButton('load theme', function()
			...
			self.Library:Notify(string.format('[theme] loaded theme %q', name))
		end)
		groupbox:AddButton('overwrite theme', function()
			local name = self.Library.Options.ThemeManager_CustomThemeList.Value

			self:SaveCustomTheme(name)
			self.Library:Notify(string.format('[theme] overwrote config %q', name))
		end)
		groupbox:AddButton('delete theme', function()
			...
			if not success then
				self.Library:Notify('[theme] failed to delete theme: ' .. err)
				return
			end

			self.Library:Notify(string.format('[theme] deleted theme %q', name))
			self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
			self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
		end)
		...
		groupbox:AddButton('set as default', function()
			if self.Library.Options.ThemeManager_CustomThemeList.Value ~= nil and self.Library.Options.ThemeManager_CustomThemeList.Value ~= '' then
				...
				self.Library:Notify(string.format('[theme] set default theme to %q', self.Library.Options.ThemeManager_CustomThemeList.Value))
			end
		end)
		groupbox:AddButton('reset default', function()
			local success = pcall(delfile, self.Folder .. '/themes/default.txt')
			if not success then 
				self.Library:Notify('[theme] failed to reset default: delete file error')
				return
			end
				
			self.Library:Notify('[theme] set default theme to nothing')
			self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
			self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
		end)
